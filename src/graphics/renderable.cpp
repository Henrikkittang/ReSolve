...
Renderable::Renderable()
    : m_vertexCapacity(0), m_vertexSize(0), m_stride(0), m_type(PrimitiveType::TRIANGLE), 
...
Renderable::Renderable(const void* data, uint32_t size, uint32_t floatPerVertex, PrimitiveType type, int mode)  
    : m_vertexCapacity(size), m_vertexSize(size), m_stride(floatPerVertex), m_type(type),  
...
Renderable::Renderable(const void* data, uint32_t size, uint32_t vertexCapacity, uint32_t floatPerVertex, PrimitiveType type, int mode)  
    : m_vertexCapacity(vertexCapacity), m_vertexSize(size), m_stride(floatPerVertex), m_type(type),  
      m_mode(mode), m_vertexBufferID(0), m_vertexArrayID(0), m_indexBufferID(0)
{
    ...
    GLCall(glGenBuffers(1, &m_vertexBufferID));
    GLCall(glBindBuffer(GL_ARRAY_BUFFER, m_vertexBufferID));
    GLCall(glBufferData(GL_ARRAY_BUFFER, m_vertexCapacity * m_stride, data, m_mode));
    CHECK_WARN(m_vertexBufferID == 0, "Failed to generate buffer array");
...
Renderable::Renderable(Renderable&& other)
    ...
    m_stride(other.m_stride), m_type(other.m_type), m_mode(other.m_mode),  
...
// ! vertexOffset always has to be 0
void Renderable::update(const void* data, uint32_t vertexSize, uint32_t vertexOffset)
{
    ...
    const uint32_t offset = vertexOffset * m_stride;
    const uint32_t size   = vertexSize  * m_stride;

    GLCall(glBindBuffer(GL_ARRAY_BUFFER, m_vertexBufferID));

    // GLCall(glBufferData(GL_ARRAY_BUFFER, m_vertexSize * m_stride, data, m_mode));
...
void Renderable::updateAppend(const void* data, uint32_t vertexSize) 
{
    ...
    const uint32_t offset = m_vertexSize * m_stride;
    const uint32_t size   = vertexSize  * m_stride;
